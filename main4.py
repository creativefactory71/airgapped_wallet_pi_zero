...
from pin_manager import setup_or_verify_pin  # and reset_pin will be used later
...
class MenuState:
    ...
    CONFIRM_DELETE = 9
    SETTINGS = 10  # <-- New state for Settings
...
class WalletUI:
    def __init__(self):
        ...
        self.settings_menu = ["Reset PIN", "Back"]  # <-- New Settings Menu
    ...
    def handle_button_press(self, button):
        ...
        if self.current_state == MenuState.VERIFY_SEED:
            ...
            handle_typing_input(self, button)
            return
    ...
    def handle_up_down(self, button):
        if self.current_state == MenuState.HOME_SCREEN:
            ...
            if button == "UP":
                self.selected_option = (self.selected_option - 1) % len(self.settings_menu)
            elif button == "DOWN":
                self.selected_option = (self.selected_option + 1) % len(self.settings_menu)
    ...
    def update_settings_display(self):
        show_text_highlighted(self.settings_menu, self.selected_option)
        self.current_state = MenuState.SETTINGS
    def execute_settings_option(self):
        from pin_manager import reset_pin  # Import inside function (safe)

        option = self.settings_menu[self.selected_option]
        if option == "Reset PIN":
            reset_pin()
            self.go_back_to_main()
        elif option == "Back":
    ...
    def handle_send_transaction_input(self, button):
        if button == "ENTER":
            if self.tx_input_field == "receiver":
                ...
                chain_id = 50  # Default XDC Chain ID
                if self.selected_coin == "Ethereum":
                    chain_id = 11155111  # Example Ethereum Chain ID
...
if __name__ == "__main__":
    try:
        setup_or_verify_pin()  # <-- Check or set PIN first (hardware keypad)
        wallet_ui = WalletUI()
