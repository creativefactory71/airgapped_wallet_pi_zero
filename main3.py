...
class MenuState:
    ...
    HOME_SCREEN = 8
    SELECT_CRYPTO = 9
    SEND_TRANSACTION_INPUT = 10
    SEND_TRANSACTION_CONFIRM = 11
...
class WalletUI:
        ...
        if self.current_state == MenuState.DISPLAY_SEED:
            ...
            if button == "ENTER":
                if self.tx_input_field == "receiver":
                    ...
                    self.current_state = MenuState.SEND_TRANSACTION_CONFIRM
            ...
            if button == "UP":
                self.selected_option = (self.selected_option - 1) % len(self.confirm_menu)
            elif button == "DOWN":
                self.selected_option = (self.selected_option + 1) % len(self.confirm_menu)
            elif button == "ENTER":
                if self.confirm_menu[self.selected_option] == "Yes":
    ...
    def update_transaction_input_display(self):
        if self.tx_input_field == "receiver":
            ...
            show_text_highlighted(["Amount (XDC):", self.tx_amount, "Press ENTER"], -1)
